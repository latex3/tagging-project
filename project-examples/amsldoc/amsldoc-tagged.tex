...
\newif\iftaggingSE
\newif\iftaggingAF
%tagging: ======= added: use \DocumentMetadata
\DocumentMetadata{
 lang=en, 
 tagging=on,
...
%tagging: ======== added: undefine the commands:
\let\openbox\relax\let\qedsymbol\relax
%endtagging ========

\documentclass[leqno,titlepage,openany]{amsldoc}[1999/12/13]

%tagging: ======== added: load unicode math fonts
\usepackage{unicode-math} %lua-unicode-math errors because of missing varGamma
...
%tagging: =========== added: \@starttoc needs the configuration points
\makeatletter
...
%tagging: =========  added: redefine ctag
% Don't use a triv list with ctab and do not tag the table as table
\renewenvironment{ctab}[1][table-tagging=presentation]{%
...
% tagging: ===== added: artifact the rules of fbox. 
% This is luatex only! 
\makeatletter
\def\@frameb@x#1{%
  \@tempdima\fboxrule
  ...
  \hbox{%
    \lower\@tempdima\hbox{\tagmcbegin{artifact}%
      \vbox{%
...
%tagging: ========== added: actualtext around the symbol
\renewcommand{\errorbullet}{%
   \UseTaggingSocket{inline/begin}{tag=Span,actualtext=⚠}%
   \rule[-.5pt]{2.5pt}{7.5pt}%
...
%tagging: ======== removed: breaks, 
%the new code does not expect `\chaptername` to take an argument
%\renewcommand{\chaptername}[1]{}
%end tagging =============

%tagging: ======== removed, does nothing in the new code. 
...
%tagging: ========== added: reinstate chapter layout with new methods
\EditInstance{heading}{chapter}
{
  ,prefix=
  ,number-format= \mdash\theheading\mdash
  ,heading-decls = \raggedright \parindent 0pt\bfseries \large
  ,title-decls = \large
}
...
% tagging ========
\iftaggingSE
\\\textbf{Suppressed as tagging with MathML-SE incorrect}\\
\url{https://github.com/latex3/tagging-project/issues/1033} 
\else 
\begin{equation}\label{xx}
\begin{split}
a& =b+c-d\\
...
% tagging ========
\iftaggingAF
\\\textbf{Suppressed as tagging with MathML-AF incorrect}\\
\url{https://github.com/latex3/tagging-project/issues/1033} 
\else 
\begin{align}
...
%tagging: =========== changed. This is a real table
\begin{ctab}[table-header-rows=1]{lll|lll}
...
\begin{center}
%tagging: ============ changed. minipage in tabular doesn't work well
%\begin{tabular}{@{}l@{}l@{}}
...
%tagging: ======== changed tabbing into ctab as tabbing is not yet supported
\begin{ctab}{ll}
...
\iftaggingSE
\\\textbf{Suppressed as tagging with MathML-SE incorrect}\\
\url{https://github.com/latex3/tagging-project/issues/919} 
\else 
\begin{equation}
...
%tagging: ======== changed, is a real tabular  
\begin{ctab}[table-header-rows=2]{l|llllll}
Delimiter&
  text& \ncn{left}& \ncn{bigl}& \ncn{Bigl}& \ncn{biggl}& \ncn{Biggl}\\
size&
  size& \ncn{right}& \ncn{bigr}& \ncn{Bigr}& \ncn{biggr}& \ncn{Biggr}\\
...
%tagging: ========= changed minipage in tabular replaced by p-column
\begin{ctab}{m{.6\columnwidth}l}
...
can write
%tagging: ========= avoid minipage in table
\begin{ctab}{m{.6\columnwidth}l}
%\begin{minipage}[t]{.6\columnwidth}
\begin{verbatim}
\sideset{}{'}
...
\iftaggingSE
\\\textbf{Suppressed as tagging with MathML-SE incorrect}\\
\url{https://github.com/latex3/tagging-project/issues/1437} 
\else 
\iftaggingAF
\\\textbf{Suppressed as tagging with MathML-AF incorrect}\\
\url{https://github.com/latex3/tagging-project/issues/1437} 
