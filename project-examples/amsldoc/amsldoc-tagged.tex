...
% enable if luamml should try to add structelem.
% errors currently!!
%\AssignSocketPlug{tagsupport/math/end}{luamml}

% ============================
% =========== changed ===========
% * Index heading is missing a target.
\makeatletter
\renewenvironment{theindex}
               {\if@twocolumn
                ...
                \else
                  \@restonecoltrue
                \fi
\twocolumn[{\MakeLinkTarget[chapter]{}\@makeschapterhead{\indexname}}]%
...
% * \@starttoc needs the configuration points
...
% fbox. This is luatex only! with pdflatex it is much more difficult to get 
% the mc-literal correct
\makeatletter
\def\@frameb@x#1{%
  \@tempdima\fboxrule
...
% lazy setting of mc as we are using luatex!
\renewcommand{\errorbullet}{\tagstructbegin{tag=Span}\tagmcbegin{actualtext=⚠}\rule[-.5pt]{2.5pt}{7.5pt}%
  \rule[-.5pt]{5pt}{2.5pt}\kern-2.5pt%
  \rule[4.5pt]{2.5pt}{2.5pt}\tagstructend\tagmcbegin{}}
...
% ======= changed imakeidx does not work ========
\usepackage{makeidx}
% =============================
...
\hypersetup{pdfborder=0 0 0,pdftitle=amsldoc}
\makeatletter
\let\oldcs\cs
...
\tagpdfsetup{newattribute = {list}{/O /List /ListNumbering/Unordered},}    
...
% ======== changed. List with empty body doesn't work well
\begin{description}
\item[\opt{alignedleftspaceyes}]\mbox{}
\item[\opt{alignedleftspaceno}]\mbox{}
\item[\opt{alignedleftspaceyesifneg}]\mbox{}
\end{description}
%================
...
\begin{center}
% ============ changed. minipage in tabular doesn't work well
%\begin{tabular}{@{}l@{}l@{}}
...
%======== changed tabbing into ctab as tabbing is not yet supported
\begin{ctab}{ll}
...
% =========== changed: \samepage -> \begin{samepage} for proper doendpe
\begin{samepage}
\begin{verbatim}
\cfrac{1}{\sqrt{2}+
 \cfrac{1}{\sqrt{2}+
...
% ========= changed minipage in tabular replaced by p-column
